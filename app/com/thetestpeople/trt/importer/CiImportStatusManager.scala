...
class CiImportStatusManager @Inject() (clock: Clock) {
  ...
  def buildExists(id: Id[CiImportSpec], buildUrl: URI, buildNumberOpt: Option[Int] = None, buildNameOpt: Option[String] = None) = lock.withLock {
    specStatuses(id).buildExists(buildUrl, buildNumberOpt, buildNameOpt)
  }
  ...
  private class MutableCiSpecImportStatus(specId: Id[CiImportSpec], jobUrl: URI) {
    ...
    def buildExists(buildUrl: URI, buildNumberOpt: Option[Int], buildNameOpt: Option[String]) = {
      buildStatuses += buildUrl -> new MutableCiBuildImportStatus(buildUrl, buildNumberOpt, buildNameOpt)
    }
  ...
  private class MutableCiBuildImportStatus(val buildUrl: URI, val buildNumberOpt: Option[Int], val buildNameOpt: Option[String]) {
    ...
    def snapshot = CiBuildImportStatus(buildUrl, buildNumberOpt, buildNameOpt, updatedAt, state)

  }

}

case class CiBuildImportStatus(buildUrl: URI, buildNumberOpt: Option[Int], buildNameOpt: Option[String], updatedAt: DateTime, state: BuildImportState)
